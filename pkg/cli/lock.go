...
func Lock() *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:     "lock",
		Short:   "Lock a package from a YAML configuration file",
		Long:    `Lock a package from a YAML configuration file.`,
		Example: `  melange lock [config.yaml]`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			return LockCmd(cmd.Context(), cmd.OutOrStdout(), archs, options...)
		},
	}
...
func LockCmd(ctx context.Context, w io.Writer, archs []apko_types.Architecture, baseOpts ...build.Option) error {
	ctx, span := otel.Tracer("melange").Start(ctx, "LockCmd")
	defer span.End()
	...
	if errors.Is(err, build.ErrSkipThisArch) {
		return fmt.Errorf("skipping arch %s", arch)
	} else if err != nil {
		return err
	}

	if err := bc.Lock(ctx, w); err != nil {
